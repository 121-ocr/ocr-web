...
//构建分页条件
function buildRepsQueryCond(total, pageNum, spuCate) {
    var condition = {
        paging: {
            sort_field: "_id",
            sort_direction: -1,
...
var currentSpuCate;
function loadSpuList(data){

    currentSpuCate = data;
    var condStr = buildRepsQueryCond(0,1, currentSpuCate.code);

    //定义查询条件
    $.ajax({
        method : 'POST',
        url : $apiRoot + "ocr-op-product-center/product-spu/findall?token=" + window.$token,
        async : true,
        data: condStr,
        dataType : 'json',
            ...
            $('#dgList').datagrid('getPager').pagination({
                ...
                onSelectPage : function(pPageIndex, pPageSize) {
                    if(isNewRep){
                        ...
                        condStr = buildRepsQueryCond(0, pPageIndex, currentSpuCate.code);
...
function bindDgListData(data){
    ...
    for ( var i in data.datas) {
        ...
        var row_data = {
            spu_code: dataItem.spu_code,
            name: dataItem.name,
            brand_name: dataItem.brand.name,
            is_kit: dataItem.is_kit,
            obj: dataItem
        };
...
function loadDgList(){
    ...
    $.ajax({
        method : 'GET',
        url :  $apiRoot + 'ocr-op-product-center/product-category-tree/query?token=' + window.$token,
        async : true,
        dataType : 'json',
        beforeSend: function (x) { x.setRequestHeader("Content-Type", "application/json; charset=utf-8"); },
        success : function(data) {
            if (data.errCode != undefined && data.errCode != null) {
                ...
                //success(data.result);
                $('#spuCateTree').tree('loadData', convertToSpuCateTree(data.result));
...
function convertToSpuCateTree(data){
    var ret = [];
    for ( var i in data) {
        var dataItem = data[i];
        var item = {
        ...
        if("child" in dataItem){
            item.children = convertToSpuCateTree(dataItem.child);
        }
        ret.push(item);
    }
    return ret;
