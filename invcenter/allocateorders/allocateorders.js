...
function save(){
    ...
    if(isHeadChanged || isBodyChanged || isNewRep){
        $.ajax({
            method: 'POST',
            url: $apiRoot + "ocr-inventorycenter/allocateorders-mgr/create?token=" + window.$token,
...
function onEndEdit(index, row) {
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.num = parseFloat(newValue); //设置当前行的数量值
        currentDetailRowObj.num = parseFloat(newValue); //设置当前行对象的值
    }
    ...
    //for ( var i in data) {
      ...
      //    var x= dataItem.warehouse;
        ...
        var row_data = {
            ...
            busidate:dataItem.busidate,
            //warehouse:dataItem.warehouse,
            outorg:dataItem.outorg.name,
            outwarehouses:dataItem.outwarehouses.name,
            inorg:dataItem.inorg.name,
...
function loadDgList(){
    ...
    $.ajax({
        method : 'POST',
        url : $apiRoot + "ocr-inventorycenter/allocateorders-mgr/query?token=" + window.$token,
        async : true,
        data: condStr,
        dataType : 'json',
        beforeSend: function (x) { x.setRequestHeader("Content-Type", "application/json; charset=utf-8"); },
...
function bindSelectedDataToCard(data){
    ...
    $('#outwarehouses').textbox('setValue',data.outwarehouses.name);
...
function onbusidateChanged(newValue,oldValue){
    if(initialized) return;
    cloneAllotInvObj.busidate = newValue;
    isBodyChanged = true;

    //-------刷新关联属性------
    updateParentListRow('busidate', newValue);
}
...
//仓库选择
function onInWarehoseSelected(record){
    if(initialized) return;
    cloneAllotInvObj.inwarehouses = {
        code: record.code,
        name: record.name
    };
    isBodyChanged = true;
    //-------刷新关联属性------
    updateParentListRow('inwarehouses', record.name);
}

function onOutWarehoseSelected(record){
    if(initialized) return;
    cloneAllotInvObj.outwarehouses = {
        code: record.code,
        name: record.name
    };
    isBodyChanged = true;
    //-------刷新关联属性------
    updateParentListRow('outwarehouses', record.name);
}
...
function onOutOrgSel(record){
    if(initialized) return;
    cloneAllotInvObj.outorg = record.attributes;

    isBodyChanged = true;

    //-------刷新关联属性------
    updateParentListRow('outorg', record.attributes.name);

}

function onInOrgSel(record){
    if(initialized) return;
    cloneAllotInvObj.inorg = record.attributes;

    isBodyChanged = true;

    //-------刷新关联属性------
    updateParentListRow('inorg', record.attributes.name);
...
function removeRep(){
    ...
    $.messager.confirm('删除警告', '是否确认删除?', function(r){
        if (r){
            ...
            $.ajax({
                method: 'POST',
                url: $apiRoot + "ocr-inventorycenter/allocateorders-mgr/remove?token=" + window.$token,
  ...
  if (allotInvObjIndex == undefined || allotInvObjIndex == null){return}  
   ...
   $.messager.confirm('提示', '是否确认?', function(r){
        if (r){
            ...
            $.ajax({
                method: 'POST',
                url: $apiRoot + "ocr-inventorycenter/allocateorders-mgr/confirm?token=" + window.$token,
                data: param,
                async: true,
                dataType: 'json',
                beforeSend: function (x) {
