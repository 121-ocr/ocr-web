...
function save(){
    ...
    if(isHeadChanged || isBodyChanged || isNewRep){
        $.ajax({
            method: 'POST',
            url: $invcenterURL + "ocr-inventorycenter/allocateorders-mgr/create?context=" + $token,
...
function loadDgList(){
    ...
    $.ajax({
        method : 'POST',
        url : $invcenterURL + "ocr-inventorycenter/allocateorders-mgr/query?context=" + $token,
        async : true,
        data: condStr,
        dataType : 'json',
        beforeSend: function (x) { x.setRequestHeader("Content-Type", "application/json; charset=utf-8"); },
...
function removeRep(){
    ...
    $.messager.confirm('删除警告', '是否确认删除?', function(r){
        if (r){
            ...
            $.ajax({
                method: 'POST',
                url: $invcenterURL + "ocr-inventorycenter/allocateorders-mgr/remove?context=" + $token,
