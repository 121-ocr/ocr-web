...
function loadDgList(){
    ...
    $.ajax({
        method : 'POST',
		url : $invcenterURL + "ocr-inventorycenter/safestockwarning/query?context=" + $token,
        async : true,
        data: condStr,
        dataType : 'json',
...
function bindSelectedDataToCard(data){
    ...
    $('#onhandnum').textbox('setValue',data.onhandnum);
    $('#stockoutnum').textbox('setValue',data.stockoutnum);
    $('#isWarning').textbox('setValue',data.isWarning);
