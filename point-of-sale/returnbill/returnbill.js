...
function onEndEdit(index, row) {
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.shelf_life = newValue; //设置当前行的数量值
        currentDetailRowObj.shelf_life = newValue; //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sp_tax_type = newValue; //设置当前行的数量值
        currentDetailRowObj.supply_price.tax_type = newValue; //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sp_tax_rate = parseFloat(newValue); //设置当前行的数量值
        currentDetailRowObj.supply_price.tax_rate = parseFloat(newValue); //设置当前行对象的值
    }

    var ed = $(this).datagrid('getEditor', {
        index: index,
        field: 'sp_in_currency_type'
    });
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sp_in_currency_type = newValue; //设置当前行的数量值
        currentDetailRowObj.supply_price.price_including_tax.currency.currency_type = newValue; //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sp_in_money = parseFloat(newValue); //设置当前行的数量值
        currentDetailRowObj.supply_price.price_including_tax.currency.money = parseFloat(newValue); //设置当前行对象的值
    }


    var ed = $(this).datagrid('getEditor', {
        index: index,
        field: 'sp_in_o_currency_type'
    });
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sp_in_o_currency_type = newValue; //设置当前行的数量值
        currentDetailRowObj.supply_price.price_including_tax.original_currency.currency_type = newValue; //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sp_in_o_money = parseFloat(newValue); //设置当前行的数量值
        currentDetailRowObj.supply_price.price_including_tax.original_currency.money = parseFloat(newValue); //设置当前行对象的值
    }


    var ed = $(this).datagrid('getEditor', {
        index: index,
        field: 'sp_currency_type'
    });
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sp_currency_type = newValue; //设置当前行的数量值
        currentDetailRowObj.supply_price.price.currency.currency_type = newValue; //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sp_money = parseFloat(newValue); //设置当前行的数量值
        currentDetailRowObj.supply_price.price.currency.money = parseFloat(newValue); //设置当前行对象的值
    }

    var ed = $(this).datagrid('getEditor', {
        index: index,
        field: 'sp_o_currency_type'
    });
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sp_o_currency_type = newValue; //设置当前行的数量值
        currentDetailRowObj.supply_price.price.original_currency.currency_type = newValue; //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sp_o_money = parseFloat(newValue); //设置当前行的数量值
        currentDetailRowObj.supply_price.price.original_currency.money = parseFloat(newValue); //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sa_tax_type = newValue; //设置当前行的数量值
        currentDetailRowObj.supply_amount.tax_type = newValue; //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sa_tax_rate = parseFloat(newValue); //设置当前行的数量值
        currentDetailRowObj.supply_amount.tax_rate = parseFloat(newValue); //设置当前行对象的值
    }

    var ed = $(this).datagrid('getEditor', {
        index: index,
        field: 'sa_in_currency_type'
    });
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sa_in_currency_type = newValue; //设置当前行的数量值
        currentDetailRowObj.supply_amount.money_including_tax.currency.currency_type = newValue; //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sa_in_money = parseFloat(newValue); //设置当前行的数量值
        currentDetailRowObj.supply_amount.money_including_tax.currency.money = parseFloat(newValue); //设置当前行对象的值
    }


    var ed = $(this).datagrid('getEditor', {
        index: index,
        field: 'sa_in_o_currency_type'
    });
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sa_in_o_currency_type = newValue; //设置当前行的数量值
        currentDetailRowObj.supply_amount.money_including_tax.original_currency.currency_type = newValue; //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sa_in_o_money = parseFloat(newValue); //设置当前行的数量值
        currentDetailRowObj.supply_amount.money_including_tax.original_currency.money = parseFloat(newValue); //设置当前行对象的值
    }


    var ed = $(this).datagrid('getEditor', {
        index: index,
        field: 'sa_currency_type'
    });
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sa_currency_type = newValue; //设置当前行的数量值
        currentDetailRowObj.supply_amount.money.currency.currency_type = newValue; //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sa_money = parseFloat(newValue); //设置当前行的数量值
        currentDetailRowObj.supply_amount.money.currency.money = parseFloat(newValue); //设置当前行对象的值
    }

    var ed = $(this).datagrid('getEditor', {
        index: index,
        field: 'sa_o_currency_type'
    });
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sa_o_currency_type = newValue; //设置当前行的数量值
        currentDetailRowObj.supply_amount.money.original_currency.currency_type = newValue; //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.sa_o_money = parseFloat(newValue); //设置当前行的数量值
        currentDetailRowObj.supply_amount.money.original_currency.money = parseFloat(newValue); //设置当前行对象的值
    }
    ...
    if (ed != null && ed != undefined) {
        var newValue = $(ed.target).val();
        row.returnreason = newValue; //设置当前行的数量值
        currentDetailRowObj.returnreason = newValue; //设置当前行对象的值
    }
...
function bindDgListData(data){
    ...
    var viewModel = new Array();
     ...
     for ( var i in data.datas) {
        ...
        var row_data = {
            return_code: dataItem.return_code,
            return_date:dataItem.return_date,
            //warehouse:dataItem.warehouse,

            replenishment_id:dataItem.replenishment_id,
            replenishment_code:dataItem.replenishment_code,
            out_warehouse:dataItem.out_warehouse.name,
            channel:dataItem.channel.name,
            is_completed:dataItem.is_completed.name,
            completed_date:dataItem.completed_date,
...
function bindSelectedDataToCard(data){
    ...
    $('#return_code').textbox('setValue',data.return_code);
    $('#return_code').textbox('setValue',data.return_code);
    $('#return_code').textbox('setValue',data.return_code);
    $('#replenishment_code').textbox('setValue',data.replenishment_code);
    $('#out_warehouse').textbox('setValue',data.out_warehouse.name);
    $('#channel').textbox('setValue',data.channel.name);
    $('#is_completed').textbox('setValue',data.is_completed.name);
    $('#completed_date').textbox('setValue',data.completed_date);
...
function onreturn_codeChanged(newValue,oldValue){
    if(initialized) return;
    cloneAllotInvObj.return_code = newValue;
    isBodyChanged = true;

    //-------刷新关联属性------
    updateParentListRow('return_code', newValue);
}

function onreturn_dateChanged(newValue,oldValue){
    if(initialized) return;
    cloneAllotInvObj.return_date = newValue;
    isBodyChanged = true;

    //-------刷新关联属性------
    updateParentListRow('return_date', newValue);
}

function onreplenishment_idChanged(newValue,oldValue){
    if(initialized) return;
    cloneAllotInvObj.replenishment_id = newValue;
    isBodyChanged = true;

    //-------刷新关联属性------
    updateParentListRow('replenishment_id', newValue);
}

function onreplenishment_codeChanged(newValue,oldValue){
    if(initialized) return;
    cloneAllotInvObj.replenishment_code = newValue;
    isBodyChanged = true;

    //-------刷新关联属性------
    updateParentListRow('replenishment_code', newValue);
}

function oncompleted_dateChanged(newValue,oldValue){
    if(initialized) return;
    cloneAllotInvObj.completed_date = newValue;
    isBodyChanged = true;

    //-------刷新关联属性------
    updateParentListRow('completed_date', newValue);
}

function onOutOrgSel(record){
    if(initialized) return;
    cloneAllotInvObj.channel = record.attributes;
...
function onis_completedSelected(record){
    if(initialized) return;
    cloneAllotInvObj.is_completed = {
        code: record.code,
        name: record.name
    };
    isBodyChanged = true;
    //-------刷新关联属性------
    updateParentListRow('is_completed', record.name);
}
...
//仓库选择
function onOutWarehoseSelected(record){
    if(initialized) return;
    cloneAllotInvObj.out_warehouse = {
        code: record.code,
        name: record.name
    };
    isBodyChanged = true;
    //-------刷新关联属性------
    updateParentListRow('out_warehouse', record.name);
}
...
function onWarehoseSelected(record){
    ...
    row['stocking_warehouse'] = record.name;

    currentDetailRowObj.stocking_warehouse = record; //设置当前行对象的值
    // currentDetailRowObj.stocking_warehouse = record.code; //设置当前行对象的值
...
function buildLocatinByWAQueryCond(total, pageNum) {
    ...
    var warehousecode = currentDetailRowObj.stocking_warehouse.code;
...
function bindDetailData(data){
    ...
    for ( var i in data) {
        ...
        var row_data = {
            stocking_warehouse : dataItem.stocking_warehouse.name,
            ...
            /*供应价格*/
            sp_tax_type:dataItem.supply_price.tax_type,
            sp_tax_rate:dataItem.supply_price.tax_rate,
            sp_in_currency_type:dataItem.supply_price.price_including_tax.currency.currency_type,
            sp_in_money:dataItem.supply_price.price_including_tax.currency.money,
            sp_in_o_currency_type:dataItem.supply_price.price_including_tax.original_currency.currency_type,
            sp_in_o_money:dataItem.supply_price.price_including_tax.original_currency.money,
            sp_currency_type:dataItem.supply_price.price.currency.currency_type,
            sp_money:dataItem.supply_price.price.currency.money,
            sp_o_currency_type:dataItem.supply_price.price.original_currency.currency_type,
            sp_o_money:dataItem.supply_price.price.original_currency.money,

            /*供应金额*/
            sa_tax_type:dataItem.supply_amount.tax_type,
            sa_tax_rate:dataItem.supply_amount.tax_rate,
            sa_in_currency_type:dataItem.supply_amount.money_including_tax.currency.currency_type,
            sa_in_money:dataItem.supply_amount.money_including_tax.original_currency.money,
            sa_in_o_currency_type:dataItem.supply_amount.money_including_tax.original_currency.currency_type,
            sa_in_o_money:dataItem.supply_amount.money_including_tax.original_currency.money,
            sa_currency_type:dataItem.supply_amount.money.currency.currency_type,
            sa_money:dataItem.supply_amount.money.currency.money,
            sa_o_currency_type:dataItem.supply_amount.original_currency.currency_type,
            sa_o_money:dataItem.supply_amount.original_currency.money,


            returnreason:dataItem.returnreason,
...
function onLocationsSelected (index, rowData) {
    ...
    currentDetailRowObj.location = rowData;

    //-------刷新关联属性------
    var row = $('#detailDg').datagrid('getSelected');
    var index = $('#detailDg').datagrid('getRowIndex', row);

    // row['location'] = rowData.locationref_loccode;
    row.location = rowData.locationref_loccode;
    isBodyChanged = true;
