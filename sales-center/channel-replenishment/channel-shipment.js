...
function confirm(){
    if(newShipmentDetails.length > 0){
        ...
        $.ajax({
            method: 'POST',
            url: $apiRoot + "ocr-sales-center/channel-restocking/ship?token=" + window.$token,
...
function loadDgList(){
    ...
    $.ajax({
        method : 'POST',
        url : $apiRoot + "ocr-sales-center/channel-restocking/query4readyship?token=" + window.$token,
        async : true,
        data: condStr,
        dataType : 'json',
